...
void setUpCredits() {
	...
	char* praTitles[praT] = { "SVEDKA", "GREY GOUSE", "THREE OLIVES" ,"EFFEN","SMIRNOFF"};
...
static int __cdecl randVoice(int a) {
	srand(rand() *time(NULL));
	int r = rand() % 2727;
	if (cVoices) r = storeVL[a];
	else if (vlRandoType == 1) {
		while (isOmochaoLine(r)) { r = rand() % 2727; }
		//return r;
	}
	else if (vlRandoType == 2) r = returnSameCharLine(a);
	else if (vlRandoType == 3) r = returnMainCharLine(a);
	if ((subNumber == 2 && !isOmochaoLine(a)) || (subNumber == 1 && GameState == 0)) {
		displayMe = displaySub(r, 3);
	}
...
int __cdecl LoadStartPosition_ri(int playerNum, NJS_VECTOR *position, Rotation *rotation)
{
	...
	if ( v1 )
	{
		...
		if ( v4 )
		{
			if(rSize) if (MainCharObj1[playerNum])MainCharObj1[playerNum]->Scale = { r, r, r };
...
extern "C"
{
	...
	__declspec(dllexport) void Init(const char *path, const HelperFunctions &helperFunctions)
	{
		...
		subType = settings->getString("OnOff", "subtitles");
		...
		string s = settings->getString("rando", "vlType");
		
		if(s.compare("orand") == 0) vlRandoType = 1;
		if(s.compare("samerand") == 0) vlRandoType = 2;
		if(s.compare("mainrand") == 0) vlRandoType = 3;
		...
		randoSV = true;
		rVoices = settings->getBool("rando", "rVL");
		cVoices = settings->getBool("rando", "cVL");
		...
		if(subNumber > 0) WriteData((char*)0x60221D, (char)0xEB); //Disable cutscene subs
		...
		WriteJump(BrokenDownSmokeExec, DeleteObject_);
